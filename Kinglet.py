...
#            mylogger("Error writing to system telemetry log [INIT]")
    def run(self):
        con = sqlite3.connect('kinglet.dbz')
        while mySettings.PowerOn:
            try:
#                outputFile = open(self.telem_file_name, 'a')
                cur = con.cursor()
                cur.execute('insert into perfdb values (?, ?, ?, ?, ?)', (
                    datetime.datetime.now(),
                    self.get_cpu_usage(),
                    self.get_mem_usage(),
                    str(self.get_brd_temp())[0:5]+"F",
                    str(self.get_disk_percent()) ) )
                con.commit()
#                try:
...
def mylogger(logd):
    ...
    #logfile.close()
    #add to sqlite3 logdb
    con = sqlite3.connect('kinglet.dbz')
    cur = con.cursor()
    cur.execute('insert into logdb values (?, ?)', (xd, logd))
    con.commit()
    con.close()
